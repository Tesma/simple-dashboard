const main_cat = document.getElementById('main-cat')
const sec_cat = document.getElementById('sec-cat')
...
bt_cats = [ 
    ...
    {
        name: 'privacy',
        icon: '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="M440-280h80v-240h-80v240Zm68.5-331.5Q520-623 520-640t-11.5-28.5Q497-680 480-680t-28.5 11.5Q440-657 440-640t11.5 28.5Q463-600 480-600t28.5-11.5ZM480-80q-139-35-229.5-159.5T160-516v-244l320-120 320 120v244q0 152-90.5 276.5T480-80Zm0-84q104-33 172-132t68-220v-189l-240-90-240 90v189q0 121 68 220t172 132Zm0-316Z"/></svg>',
        cat: sec_cat,
    }
]

function CreateHoverTooltip(_name,_master,_id) {
    let span_id = _id
    ...
    _master.onmouseenter = () => {
        const rect = _master.getBoundingClientRect()
        span.style.top = `${rect.top + rect.height / 2}px`
        span.style.left = `${rect.right + 8}px`
        span.style.transform = 'translateY(-50%)'
        span.style.display = 'flex'
    }
    _master.onmouseleave = () => {
        span.style.display = 'none'
    }
}

bt_cats.forEach(element => {
    let span_id = `${element.name}-sdbt-tooltip`
    let element_id = `${element.name}-sdbt`
    let cats = element.cat

    let btn = document.createElement('button')
    btn.id = element_id
    btn.className = element.class

    btn.innerHTML += element.icon

    cats.appendChild(btn)

    CreateHoverTooltip(element.name,btn,span_id)
});

let profile_bts = [
    {
        id: document.getElementById('pf-new-bt'),
        name: 'new',
    },
    {
        id: document.getElementById('pf-upload-bt'),
        name: 'upload',
    },
    {
        id: document.getElementById('pf-share-bt'),
        name: 'share'
    }
]

profile_bts.forEach(element => {
    let span_id = `${element.name}-pf-bt-tooltip`
    CreateHoverTooltip(element.name,element.id,span_id)
})
...
const projects_element = document.getElementById('projects-grid')

projects.forEach(element => {
    let project = document.createElement('div')
    project.className = 'project'
    project.innerHTML = `
        <h1 title='${element.title}'>${element.title}</h1>
        <div class='line'></div>
        <p>${element.desc}</p>
        <div class='projects-bt'>
    ...
    projects_element.appendChild(project)
})
...
const announces_ele = document.getElementById('announce')
...
announces.forEach(element => {
    ...
    switch (element.level) {
        case 0:
            level.innerText += 'VERY IMPORTANT'
            level.className = 'vimportant'
        break;

        case 1:
            level.innerText += 'IMPORANT'
            level.className = 'important'
        break;

        case 2:
            level.innerText += 'USEFUL'
            level.className = 'useful'
        break;
    }
    ...
    announce.appendChild(level)
    announce.appendChild(desc)
    announces_ele.appendChild(announce)
});

const trending_ele = document.getElementById('trending')

trending_array = [
    {
        id:'@jackmiller',
        pfp: './jack-miller.png',
        project: 'World Saver App'
    },
    {
        id:'@bp',
        pfp: './bp-pfp.png',
    ...
    },
    {
        id:'@jeffc',
        pfp: './blank-pfp.png',
        project: 'Slow Traffic Fix'
    },
]

trending_array.forEach(element => {
    const trend = document.createElement('div')
    const trend_id = document.createElement('strong')
    const txt = document.createElement('div')
    const project = document.createElement('strong')
    const pfp = document.createElement('img')

    pfp.src = element.pfp
    pfp.className = 'circle-img'
    trend_id.innerText = element.id
    project.innerText = element.project
    project.style = 'color: var(--td-color)'

    txt.appendChild(trend_id)
    txt.appendChild(project)

    trend.appendChild(pfp)
    trend.appendChild(txt)
    
    trending_ele.appendChild(trend)
});